...
オブジェクトは `beginPath` を活用して形状を作ります。
...
3. **アセット未発見時の回避策**: ロードエラーが発生した際や、特定のタイル画像がまだ存在しない場合のフォールバックとして使えます。
...
答えは**グリッド座標からピクセル座標への変換を1回で済ませるため**。`translate` しないと各 `fillRect` の中で毎回 `x * tileSize + ...` って計算しないといけない。`translate` で原点をずらしておけば、あとは `(0, 0)` 基準で描くだけでいい。
...
`restore` だけじゃ「どこに戻るか」がわからないので `save` が必要。タイル1個描くたびに座標をリセットできる仕組み。CSSの `transform` と同じ発想。
...
これらのテクニックを組み合わせることで、画像アセットが一切なくても、十分に「ゲームらしい」画面を作り上げることが可能です。
