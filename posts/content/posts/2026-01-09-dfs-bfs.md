...
これをフラットな配列 `[1, 4, 5, 6, 7, 8, 2, 3]` にしたい。

このとき、「どの順番で要素を取り出すか」がDFS/BFSの違いだ。
...
for item in data:
    if isinstance(item, list):
        for sub in item:  # ネストが2層までしか対応できない
            print(sub)
```

入れ子の深さが不明な場合、二重ループでは対応できない。
...
for i in range(N):
    queue_list.append(i)
    if len(queue_list) > QSIZE:
        queue_list.pop(0)  # O(n)
